...
def initial_flag(left, top, right, bottom):
    ...
    print("Shelf 5: Filter in IOU = {:4f}".format(calcIOU1))
    distance = distanceCal(bottom_n, bottom)
    print("shelf 5: distance = {}".format(distance))
...
def initial_flag_out(left, top, right, bottom):
    left_x, top_y, right_m, bottom_n = shangpin_area()
    calcIOU1 = newcalcIOU(left, top, right, bottom, left_x, top_y, right_m, bottom_n)
    print("Shelf 5: Filter out IOU = {}".format(calcIOU1))
    distance = distanceCal(bottom_n, bottom)
    print("shelf 5: distance = {}".format(distance))
...
def initial_flag_huojia2(left, top, right, bottom):
    ...
    calcIOU1 = newcalcIOU(left, top, right, bottom, left_x, top_y, right_m, bottom_n)
    print("Shelf 6: Filter in IOU = {:4f}".format(calcIOU1))
    distance = distanceCal(bottom_n, bottom)
    print("shelf 6: distance = {}".format(distance))
    if calcIOU1 > 0.5 and distance < 100:
        flag = 1
    else:
        flag = 0
...
def initial_flag_out_huojia2(left, top, right, bottom):
    left_x, top_y, right_m, bottom_n = shangpin_area_huojia2()
    calcIOU1 = newcalcIOU(left, top, right, bottom, left_x, top_y, right_m, bottom_n)
    print("Shelf 6: Filter out IOU = {:4f}".format(calcIOU1))
    distance = distanceCal(bottom_n, bottom)
    print("shelf 6: distance = {}".format(distance))
    if calcIOU1 > 0.2 and distance < 100:
        flag = 1
...
def people_list(res):
    ...
    for b_b in res:
        ...
        peolist.append(left)
        peolist.append(top)
        peolist.append(right)
        peolist.append(bottom)
    return peolist
...
def callback(param_tuple):  # param_tuple
    ...
    cv2.imshow('Cam7', cv2.resize(frame, (int(512 * wh_ratio), 512)))
...
def main(camera_id):
    ...
    threadPubMsg_shelfID_5 = pubmsg.MsgPublishClass(cameraID=camera_id, shelfID=5)
    threadPubMsg_shelfID_5.setDaemon(True)
    threadPubMsg_shelfID_5.start()

    threadPubMsg_shelfID_6 = pubmsg.MsgPublishClass(cameraID=camera_id, shelfID=6)
    threadPubMsg_shelfID_6.setDaemon(True)
    threadPubMsg_shelfID_6.start()

    threadPubMsg_dict = {'shelfID_5': threadPubMsg_shelfID_5, 'shelfID_6': threadPubMsg_shelfID_6}
    ...
    while not rospy.is_shutdown():
        ...
        print("**********************")
        print("dic_change shelf5: {}".format(dic))
        print("dic_change_shelf6: {}".format(dic_huojia2))
        print("")
        dic_change = dic
