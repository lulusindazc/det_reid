...
def reid_draw(frame, b_b, model, cfg, shujuku, threadPubMsg, camera_id):
    ...
    if flag_check:
        threadPubMsg.set_customer(name=customer_name, person_id=1, camera_id=camera_id, x=left, y=top, w=right, h=bottom)
    ...
    if len(res) == 1:
        ...
        for item in res:
            ...
            if (len(result) > 0):
                frame = reid_draw(frame, result, model, cfg, shujuku, threadPubMsg, camera_id)
                # flag[0] = reid_draw(frame, result, model, cfg, shujuku, threadPubMsg, camera_id, flag[0])
    cv2.imshow('Cam4', cv2.resize(frame, (int(512 * wh_ratio), 512)))
...
def main(camera_id):
    ...
    rospy.Subscriber(frame_topic, CompressedImage, callback, (threadPubMsg, cfg, model, shujuku, frame_number2, bridge, camera_id))
