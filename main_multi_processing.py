...
global filterout_count
filterout_count = 0

def reid_draw(frame, b_b, model, cfg, huojia1_id, huojia2_id, pre_res):
    global size
    global filterout_count
    id_name = 0
    cfg.cuda()
    ...
    if bottom > 530:
        ...
        #     cv2.imwrite('/home/shiw/yhr/filtered_out/' + str(filterout_count)+ '.jpg', img1)
        #     filterout_count += 1
        #     return left, top, right, bottom, 999
    ...
    left_x_2, top_y_2, right_m_2, bottom_n_2 = shangpin_area(huojia2_id)
...
def initial_flag_out(left, top, right, bottom, shelfid):
    ...
    distance = distanceCal(bottom_n, bottom)
    print("shelf {}: distance = {}".format(shelfid, distance))
    if calcIOU1 > 0.2 or distance < 100:
        flag = 1
    else:
        flag = 0
    return flag
...
def xuanze_original(res, frame, model, cfg, camera_id, dic_change, dic_change_huojia2, huojia1_id, huojia2_id,pre_res):
    dic = {}
    dic_huojia2 = {}
    if len(res) == 1:
        result = res[0]
        left, top, right, bottom, id_name = reid_draw(frame, result, model, cfg, huojia1_id, huojia2_id,pre_res)
        if id_name == 999:
            None
        else:
            ...
            if dic_change_huojia2.has_key(id_name):
                if dic_change_huojia2[id_name] == 1:
                    flag = initial_flag_out(left, top, right, bottom, huojia2_id)
                else:
                    flag = initial_flag(left, top, right, bottom, huojia2_id)
            else:
                flag = initial_flag(left, top, right, bottom, huojia2_id)
...
def xuanze(res, frame, model, cfg, threadPubMsg_dict, camera_id, dic, change_dic, dic_huojia2, change_dic_huojia2,
           huojia1_id, huojia2_id, frame_trans,pre_res):
    global in_out_moments_count
    for item in res:
        result = item
        # add new person
            ...
            if id_name == 999:
                ...
                if id_name in in_out_people:
                    ...
                    if change_dic[id_name] == 1:
                        ...
                        cv2.imwrite('/home/shiw/yhr/in_out_moments/' + str(in_out_moments_count) + str('_out') + '.jpg',
                                    frame)
                        ...
                        cv2.imwrite('/home/shiw/yhr/in_out_moments/' + str(in_out_moments_count) + str('_in') + '.jpg',
                                    frame)
                        in_out_moments_count += 1
                ...
                if id_name in in_out_people_huojia2:
                    left_x, top_y, right_m, bottom_n = shangpin_area(huojia2_id)
                    customer_name = "name" + str(id_name)
                    assert type(id_name) is int  # must be number
                    # print("set customer message")
                    ...
                    if change_dic_huojia2[id_name] == 1:
                        ...
                        threadPubMsg.set_commodity_recognition_trigger_with_image(camera_id=camera_id,
                                                                                  ...
                                                                                  flag_list=people_list(res, huojia2_id),
                                                                                  frame=frame_trans)
                        cv2.imwrite('/home/shiw/yhr/in_out_moments/' + str(in_out_moments_count) + str('_out') + '.jpg', frame)
                        in_out_moments_count += 1
                        ...
                        threadPubMsg.set_commodity_recognition_trigger_with_image(camera_id=camera_id,
                                                                                  ...
                                                                                  flag_list=people_list(res, huojia2_id),
                                                                                  frame=None)
                        cv2.imwrite('/home/shiw/yhr/in_out_moments/' + str(in_out_moments_count) + str('_in') + '.jpg', frame)
                        in_out_moments_count += 1
...
def callback(param_tuple):  # param_tuple
    ...
    huojia2_id = param_tuple[9]
    pre_res = param_tuple[10]
    shape = frame.shape
...
def main(camera_id, shelf_id):
    ...
    threadPubMsg_shelfID_2 = pubmsg.MsgPublishClass(cameraID=camera_id, shelfID=shelf_id[1])
    threadPubMsg_shelfID_2.setDaemon(True)
    threadPubMsg_shelfID_2.start()

    shelf1 = 'shelfID_' + str(shelf_id[0])
    shelf2 = 'shelfID_' + str(shelf_id[1])
    threadPubMsg_dict = {shelf1: threadPubMsg_shelfID_1, shelf2: threadPubMsg_shelfID_2}
    ...
    while not rospy.is_shutdown():
        ...
        res, camera_id, dict_res = callback((None, cfg, model, frame_number2, bridge, camera_id, flag, frame_origin, huojia1_id, huojia2_id,pre_res))
        ...
        dic, dic_huojia2 = xuanze_original(res, frame_origin, model, cfg, camera_id, dic_change,
                                           dic_change_huojia2, huojia1_id, huojia2_id,pre_res)
        ...
        if compare_dic(dic, dic_change) == False and compare_dic(dic_huojia2, dic_change_huojia2) == False:
            ...
            dic, dic_huojia2 = xuanze(res, frame_origin, model, cfg, threadPubMsg_dict, camera_id, dic, dic_change,
                                      dic_huojia2, dic_change_huojia2, huojia1_id, huojia2_id, frame_trans,pre_res)
            pre_res = {}
        print("**********************")
        print("dic_change_shelf_{}: {}".format(shelf_id[0], dic))
        print("dic_change_shelf_{}: {}".format(shelf_id[1], dic_huojia2))
        print("")
        dic_change = dic
